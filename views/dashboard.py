...
def show_dashboard():
    ...
    with tab5:
        show_lopende_rekening_tab(transactions, db_ops, user.id)
...
def show_empty_state():
    ...
    if st.button("📤 Ga naar CSV Importeren", type="primary"):
        ...
        st.rerun()
def show_lopende_rekening_tab(all_transactions: list, db_ops: DatabaseOperations, user_id: str):
    ...
    # Filter locally since we fetched all transactions already
...
def show_lopende_rekening_tab(all_transactions: list, db_ops: DatabaseOperations, user_id: str):
    ...
    if not lopende_trans:
        ...
        return
    # Calculate sum
    total_open = sum(t['bedrag'] for t in lopende_trans)
    st.metric("Totaal openstaand", f"€{total_open:,.2f}", delta=None)
    st.divider()
    # Display list with uncheck option
    t_data = []
    # Custom CSS for the uncheck button
    st.markdown("""
        <style>
        .small-btn {
            padding: 0px 5px; 
    ...
    for t in lopende_trans:
        with st.container():
            ...
            with col1:
                st.write(f"**{t['naam_tegenpartij'] or 'Onbekend'}**")
                st.caption(t.get('omschrijving', ''))
            with col2:
                st.write(f"€{t['bedrag']:,.2f}")
            with col3:
                st.write(t['datum'].strftime('%d-%m-%Y') if hasattr(t['datum'], 'strftime') else t['datum'])
            with col4:
                if st.button("❌", key=f"del_lop_{t['id']}", help="Markering verwijderen"):
                    if db_ops.update_transaction(t['id'], {"is_lopende_rekening": False}, user_id):
